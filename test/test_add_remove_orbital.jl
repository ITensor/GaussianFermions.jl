...
@testset "AnnihilationOperator construction methods" begin
    ...
    @test gf.correlation_matrix(ψ_direct) ≈ gf.correlation_matrix(ψ_sum) atol=1E-10
    @test gf.trace(ψ_direct) ≈ gf.trace(ψ_sum) atol=1E-10
end
...
@testset "CreationOperator construction methods" begin
    ...
    @test gf.correlation_matrix(ψ_direct) ≈ gf.correlation_matrix(ψ_sum) atol=1E-10
    @test gf.correlation_matrix(ψ_direct) ≈ gf.correlation_matrix(ψ_alt) atol=1E-10
    @test gf.trace(ψ_direct) ≈ gf.trace(ψ_sum) atol=1E-10
end
...
@testset "Annihilation: ⟨ψu|ψv⟩ = u†C₀v" begin
    ...
    @test gf.trace(ψu) ≈ real(u' * C0 * u) atol=1E-10
    @test gf.trace(ψv) ≈ real(v' * C0 * v) atol=1E-10

    # Cross term via polarization: ⟨ψu|ψv⟩ = u†C₀v
    inner_uv = (gf.trace(ψuv) - gf.trace(ψu) - gf.trace(ψv)) / 2
    @test inner_uv ≈ real(u' * C0 * v) atol=1E-10
end
...
@testset "Creation: ⟨ψu†|ψv†⟩ = u†(I−C₀)v" begin
    ...
    @test gf.trace(ψu) ≈ real(u' * IC0 * u) atol=1E-12
    @test gf.trace(ψv) ≈ real(v' * IC0 * v) atol=1E-12

    # Cross term via polarization: ⟨ψu†|ψv†⟩ = u†(I−C₀)v
    inner_uv = (gf.trace(ψuv) - gf.trace(ψu) - gf.trace(ψv)) / 2
    @test inner_uv ≈ real(u' * IC0 * v) atol=1E-12
end
