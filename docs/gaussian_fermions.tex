...
\end{align}
which is the basis for a single ``spinless fermion site'', with $\ket{0}$ being the vacuum or empty state and $\ket{1}$ being the filled state or state of one fermion. So far the fermionic nature is not very obvious except  that we can only have at most one fermion. 
...
\ket{1}_1 \otimes \ket{0}_2 & \leftrightarrow {\color{ForestGreen} +}\ket{1}_2 \otimes \ket{0}_1 \nonumber \\
\ket{0}_1 \otimes \ket{1}_2 & \leftrightarrow {\color{ForestGreen} +}\ket{0}_2 \otimes \ket{1}_1  \nonumber \\
...
\end{align}
In other words, the order in which fermionic basis vectors are arranged is important. If two odd basis vectors are permuted, the basis vector is multiplied by a minus sign. 
...
Often a tensor product of basis states such as $\ket{1}_i \otimes \ket{1}_j$ is written in the more compact notation $\ket{1_i 1_j}$. And more generally, tensor product symbols ``$\otimes$'' are dropped when understood from context.
...
One can of course define tensor products of operators in the usual way, such as $\hat{c}_1 \otimes \hat{c_2}$ or $\hat{c}^\dagger_2 \otimes \hat{c}_1$.
...
Now comes a crucial point: the creation and annihilation operators have non-trivial commutation relations, and these are entirely a consequence of the state properties. For operators on state spaces $V_i$ and $V_j$, the commutation relations are
\begin{align}
\hat{c}_i \otimes \hat{c}_j = -  \hat{c}_j \otimes \hat{c}_1 \\ 
\hat{c}^\dagger_i \otimes \hat{c}^\dagger_j = -  \hat{c}^\dagger_j \otimes \hat{c}^\dagger_i \\ 
\hat{c}_i \otimes \hat{c}^\dagger_j = -  \hat{c}^\dagger_j \otimes \hat{c}_i  \label{eqn:ccdag_relation}
\end{align}
...
For example, 
\begin{align}
\hat{c}_1  \otimes \hat{c}_2   & =  \ket{0}_1 \otimes \bra{1}_1 \otimes \ket{0}_2 \otimes \bra{1}_2 \nonumber \\
...
\end{align}
where $\Psi^{n_1 n_2 \cdots n_N}$ is an arbitrary tensor of complex amplitudes. The anti-commuting nature of the fermions is automatically satisfied by the properties of the basis, and no additional symmetry properties need to be assumed for the amplitudes $\Psi^{n_1 n_2 \cdots n_N}$. One common assumption, though, is that for a closed system the total number of particles $N_p$ is fixed, such that $n_1 + n_2 + \ldots + n_N = N_p$ otherwise $\Psi^{n_1 n_2 \cdots n_N} = 0$. (Superconducting states are a common exception to this rule, though these are probably more properly understood as mixed states.)
...
\end{align}
Above we have introduced operators such as $\hat{A} =  \sum_{i_1=1}^N A^{i_1} \hat{c}^\dagger_{i_1}$. One describes the state above by saying that it contains four fermions, which are in the ``orbitals'' $A, B, C$ and $D$. These orbitals are vectors $\vec{A}= [A^1 \ A^2 \ \ldots A^N]$ which are unit-normalized. The vectors are not necessarily orthogonal.

The Gaussian states are quite special and far from being a general fermionic state (even though they do belong to the vector space of general fermionic states). To see that the Gaussian states are not general, let us write a two-particle Gaussian state 
...
Expressing states in this basis is known as ``first quantization''.
...
The first-quantized basis can be convenient for proving facts such as if $\vec{A} = \vec{B}$, then the state $\ket{AB} = 0$. In the more general setting, this fact about orbitals required to be independent is known as the \emph{Pauli exclusion principle}. It is not a separate axiom about fermions, but follows from their vector space properties.
