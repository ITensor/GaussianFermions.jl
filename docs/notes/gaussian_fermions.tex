...
\begin{align}
\boxed{G^{R\, i}_j(t) = \theta(t)(G^{>\, i}_j(t) - G^{<\, i}_j(t)) = -i \theta(t) \sum_n \phi^i_n e^{-i \epsilon_n t} \bar{\phi}_j^n  = -i \theta(t) e^{-i h t} } \ \ .  \label{eq:GR}
\end{align}
...
\begin{align}
\boxed{ \tilde{G}^{R\, i}_j(\omega)  = \sum_n \frac{\phi^i_n \bar{\phi}_j^n}{\omega - \epsilon_n + i \alpha} = \frac{1}{(\omega + i \alpha) - h } } \ \ .
\end{align}
This form is known as the Lehmann representation. In the last expression above, the terms in the denominator are matrices and it is understood that $(\omega + i \alpha)$ is multiplied by an identity matrix.
...
\end{align}
Given a spectral decomposition of the ``hopping matrix'' $h^i_j = \sum_n \phi^i_n \epsilon_n \hat{\phi}^n_j$ and defining $\hat{\phi}^\dagger_n = \sum_j \phi^j_n \hat{c}^\dagger_j$, we can then write 
\begin{align}
...
\begin{align}
    C_i\,^j = \text{Tr}[ \hat{c}^\dagger_i \hat{c}^j ] = \left[ \frac{1}{1+e^{\beta \mathbf{h}}} \right]_i^j 
\end{align}
...
\begin{align}
\ket{\psi} = \sum_\mu w_\mu \ket{\mu}_A \ket{\mu}_B
\end{align}
...
To obtain the Schmidt decomposition, first compute the singular value decomposition of $C_{AB}$
\begin{align}
...
singular values of $C_{AB}$ through 
\begin{align}
\lambda_j = \sqrt{\nu_j (1-\nu_j)} \ \ .
\end{align}
