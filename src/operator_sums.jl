...
function process_gaussian_tuple(t::Tuple)
    ...
    if length(t) - n + 1 != 4
        error("Terms must be of the form (coef,)\"Op1\",l1,\"Op2\",l2 when adding to GaussianOperator")
    end
    op1 = t[n]; n += 1
    label1 = t[n]; n += 1
    op2 = t[n]; n += 1
    ...
    elseif is_annihilation(op1) && is_creation(op2)
        ...
        error("Operator pair ($op1, $op2) is not valid. Must be (\"Cdag\"/\"C†\", \"C\") or (\"C\", \"Cdag\"/\"C†\")")
    end
end
